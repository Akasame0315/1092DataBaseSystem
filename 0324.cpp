...
void prsub5(int k){ //某課程有哪些修課學生 
	for(int i = 0 ; i < LengthTeacher; i++)	
		if(TabTeacher[i].tid == k){
			printf("\n%s", TabCourse[(TabTeacher[i].teaching) - 1].name);
			for(int j = 0; j < LengthStudent; j++){
				if(TabStudent[j].class1 == TabCourse[(TabTeacher[i].teaching) - 1].cid || \
				TabStudent[j].class2 == TabCourse[(TabTeacher[i].teaching) - 1].cid )
					printf("\t%s", TabStudent[j].name);
			}
		}
}

void prsub6(int k){ //某學生總學分
	int StuCredit = 0;
	for(int i = 0; i < LengthStudent; i++){
		if(k == TabStudent[i].sid){
			for(int j = 0; j < LengthCourse; j++){
				if(TabCourse[j].cid == TabStudent[i].class1){
					StuCredit += TabCourse[j].credit;
				}
				if(TabCourse[j].cid == TabStudent[i].class2){
					StuCredit += TabCourse[j].credit;
				}
			}
			printf("\n%s has %d credits", TabStudent[i].name, StuCredit);
		}
	}
}
